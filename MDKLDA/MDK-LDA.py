...
class DMMmodel(object):  # modify by Pangjy 08-13
    def __init__(self, loadpath, K, opt):
        ...
        # GKLDA用的参数
        self.MS = 4 # number of must-sets
        self.tAlpha = 0
        self.sBeta = 0

        # size of must-set * gamma
        self.gamma_GKLDA = np.zeros((),dtype='float64')
        self.vGamma_GKLDA = np.zeros((),dtype='float64')

        # self.theta_GKLDA = self.theta # todo:参数重复
        self.thetasum_GKLDA = np.zeros((self.dpre.docs_count,self.K),dtype='float64')
        # self.phi_GKLDA = self.phi # todo:参数重复
        self.phisum_GKLDA = np.zeros((),dtype='float64')
