...
class LDAmodel(object):
	def __init__(self, loadpath,K):
		...
		self.topicprob=np.array([[0 for _ in range(self.K)] for x in range(self.dpre.docs_count) for y in range(self.dpre.docs[x].length)])
	...
	def sampling(self, i, j,tpidx):
		# 换主题
		word = self.dpre.docs[i].words[j]  # 获取第i个文档第j个词的编号

		Vbeta = self.dpre.words_count * self.beta
		# gibbs sample 公式（LDA数学八挂公式29）
		self.p = (self.nw[word] + self.beta) / (self.nwsum + Vbeta) * self.topicprob[tpidx]
	...
	def _theta(self):
		for i in range(self.dpre.docs_count):
			self.theta[i] = self.nd[i] / self.ndsum[i]
