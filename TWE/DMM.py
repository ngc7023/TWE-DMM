...
class DMMmodel(object):
	...
	def sampling(self, i):
		...
		for j in range(self.dpre.d76ocs[i].length): # 文档i的长度
		...
		for j in range(self.dpre.docs[i].length): # 这篇文档的单词长度
			word = self.dpre.docs[i].words[j]  # 获取第i个文档第j个词的wordix
			self.p = self.p * ((1-self.lam)*(self.nw[word] + self.beta) / (self.F + Vbeta)+self.lam*self.prob[self.probIdx][word]) # F: 每个topic词的数量; prob shape = (sample_number, word_number)某个样本下一个词是word的概率;
			self.probIdx+=1
	...
	def _theta(self): # 文章-主题分布
		...
		for i in range(self.dpre.docs_count):
			...
			for j in range(self.dpre.docs[i].length): #default_(0,length)
				word = self.dpre.docs[i].words[j]  # 获取第i个文档第j个词的编号
				theta = theta * ((1 - self.lam) * (self.nw[word] + self.beta) / (self.F + Vbeta) + self.lam * self.prob[self.probIdx][word])
				self.probIdx+=1
