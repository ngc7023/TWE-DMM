...
class DMMmodel(object):
	...
	def sampling(self, i):
		...
		for j in range(self.dpre.docs[i].length): # 文档i的长度
			word = self.dpre.docs[i].words[j]  # 获取第i个文档第j个词的编号
			self.nw[word][topic] -= 1
			self.F[topic]-=1 				   # 该topic的单词数量-1
		...
		try:
			...
			for j in range(1,self.dpre.docs[i].length): # 这篇文档的单词长度
				word = self.dpre.docs[i].words[j]  # 获取第i个文档第j个词的wordix
				self.p = self.p * ((1-self.lam)*(self.nw[word] + self.beta) / (self.F + Vbeta)+self.lam*self.prob[self.probIdx]) # F: 每个topic词的数量; prob shape = (sample_number, word_number)某个样本下一个词是word的概率;
				self.probIdx+=1;
				# print(self.probIdx);
				# print("prob:", self.prob[self.probIdx]);print("p:", self.p);
			p = np.squeeze(np.array(self.p / np.sum(self.p)))  # squeeze:去掉数组形状中单维度条目
			topic = np.argmax(np.random.multinomial(1, p))  # 以概率p的分布随机选择主题（概率p的多项式分布采样，得到一个频率的array，取频率最大的下标，也就是topic）
		except TypeError:
			print("prob:",self.prob[self.probIdx]);
		# 	print("p:",self.p);
	...
	def _phi(self): # 主题-词分布
		for i in range(self.K): # K * word_count
