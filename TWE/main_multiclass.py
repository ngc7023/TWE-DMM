...
from TWE.utils import get_minibatches_idx, restore_from_save, tensors_key_in_file, prepare_data_for_emb, \
	load_class_embedding

from TWE.evaluation import macro_f1, micro_f1, precision_at_k
...
def emb_classifier(x, x_mask, y, dropout, opt, class_penalty):
	...
	for i, ih in enumerate(H_enc_list):
		logits_list.append(
			discriminator_0layer(ih, opt, dropout, prefix = 'classify_{}'.format(i), num_outputs = 1, is_reuse = False))
...
def main():
	...
	with tf.device('/gpu:1'):
		...
		accuracy_, loss_, train_op, W_norm_, global_step, logits_, prob_ = emb_classifier(x_, x_mask_, y_, keep_prob,
																						  opt, class_penalty_)
	uidx = 0
	max_val_accuracy = 0.
	max_test_accuracy = 0.
				...
				for _, train_index in kf:
					...
					if uidx % opt.valid_freq == 0:
						...
						for _, val_index in kf_val:
							...
							val_accuracy, val_logits, val_probs = sess.run([accuracy_, logits_, prob_],
						...
						val_auc_list = []
						val_auc_micro = roc_auc_score(y_true = val_true_array, y_score = val_logits_array,
													  average = 'micro')
						val_auc_macro = roc_auc_score(y_true = val_true_array, y_score = val_logits_array,
													  average = 'macro')
						for i in range(opt.num_class):
							if np.max(val_true_array[:, i] > 0):
								val_auc = roc_auc_score(y_true = val_true_array[:, i],
						...
						if True:
							...
							for _, test_index in kf_test:
								...
								test_accuracy, test_logits, test_probs = sess.run([accuracy_, logits_, prob_],
																				  feed_dict = {x_: x_test_batch,
							...
							test_auc_list = []
							test_auc_micro = roc_auc_score(y_true = test_true_array, y_score = test_logits_array,
														   average = 'micro')
							test_auc_macro = roc_auc_score(y_true = test_true_array, y_score = test_logits_array,
														   average = 'macro')
							...
							for i in range(opt.num_class):
								if np.max(test_true_array[:, i] > 0):
									test_auc = roc_auc_score(y_true = test_true_array[:, i],
