...
def emb_classifier(x,  y, dropout, opt, class_penalty):
	...
	# 									is_reuse = False)
	prob_y=tf.nn.softmax(logits) # 返回 batch_size * wordnum
...
def main():
	...
	if opt.dataset == 'train_text':
		...
		loadpath = 'E:\TWE-DMM\data\classifydata\classifydata_index.p'
		embpath = "E:\TWE-DMM\data\classifydata\classifydata_emb.p"
	...
	train,train_lab=make_train_data(x[0],opt.maxlen) #
	...
	train_lab = np.array(train_lab, dtype = 'int')
	...
	try:
		...
		print("Load pretrained word embedding finished")
		# opt.W_class_emb = load_class_embedding(wordtoix, opt)  # 加载预训练的label embedding
	except IOError:
	...
	with tf.device('/cpu:0'):
		...
		accuracy_, loss_, train_op, W_norm_, W_class_, global_step,prob_ ,att_score = emb_classifier(x_, y_, keep_prob, opt,
																		  class_penalty_)
	uidx = 0
	...
	with tf.Session(config = config) as sess:
		...
		try:
			for epoch in range(opt.max_epochs):
				...
				tmp_prob=[]
				for i in range(len(train)): #modify
					tmp_prob.append(0)  # n_words = None tmp_prob是list,长度为样本长 #modify
				# for i in range(len(train)):
				# 	tmp_prob.append([0]*opt.n_words) # n_words = None tmp_prob是list,长度为样本长

				kf = get_minibatches_idx(len(train), opt.batch_size, shuffle = True) #zip(range(len(minibatches)), minibatches)
				for _, train_index in kf:
					uidx += 1

					sents = [train[t] for t in train_index]
					# w_labels=[predict_word[t] for t in train_index]
					x_labels = [train_lab[t] for t in train_index]
					# x_labels = np.array(x_labels)
					# x_labels = x_labels.reshape((len(x_labels), opt.num_class))
					# x_batch, x_batch_mask = prepare_data_for_emb(sents, opt)

					# sess.run([fetches1,fetches2],fedd_dict)
					_, loss, step,prob,word_emb,topic_emb,att_ = sess.run([train_op, loss_, global_step,prob_,W_norm_,W_class_,att_score],
					...
					if uidx % opt.valid_freq == 0:
						# print(sess.run(logists_, logists_class_))
						avgloss += loss
						iter_time += 1
					...
					for t in train_index:
						...
						tmp_prob[t] = prob[ai][x_labels[ai]]  # tmp_prob shape = (sample_number) # modify
						ai+=1

				saver.save(sess, opt.save_path, global_step = epoch)
				print("average loss:", avgloss / iter_time)
