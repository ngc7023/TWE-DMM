...
def emb_classifier(x,  y, dropout, opt, class_penalty):
	...
	W_class = embedding_class(y, opt, 'class_emb')  # b * e, c * e
	...
	H_enc = tf.squeeze(H_enc)  # b * e  # tf.squeeze：将原始张量中所有维度为1的那些维都删掉的结果
...
def main():
	...
	if opt.part_data:  # 是否使用部分数据（dropout使用？）
		# np.random.seed(123)
	...
	with tf.device('/cpu:0'):
		x_ = tf.placeholder(tf.int32, shape = [opt.batch_size, opt.maxlen], name = 'x_')  # 输入训练文本单词id
		# x_mask_ = tf.placeholder(tf.float32, shape = [opt.batch_size, opt.maxlen], name = 'x_mask_')
		keep_prob = tf.placeholder(tf.float32, name = 'keep_prob')
		y_ = tf.placeholder(tf.int32, shape = [opt.batch_size, ], name = 'y_')  # 标签数据
