...
def embedding(features, opt, prefix = '', is_reuse = None):
	...
	# features : [opt.batch_size, opt.maxlen] 30
	...
	with tf.variable_scope(prefix + 'embed', reuse = is_reuse):
		if opt.fix_emb:
			...
			W = tf.get_variable('W', initializer = opt.W_emb, trainable = True)  # initializer: 指定初始化方式 所有单词embedding
			print("initialize word e mbedding finished")
		else:
